...
\item a threshold $t$ that indicates the number of times a pattern should appear among all transactions in order to be frequent. The variable expresses this number with a percentage of $T$. For example having $T$ equal to 100 and $t$ equal to 0.2 means that in order for a pattern to be frequent it has to appear at least 20 times.
\end{itemize} 
...
\begin{algorithmic}[1]
\Function{GenerateIndexesDictionary}{TransactionTree[] $TPatterns$, Integer $max\_nodes$, Integer $min\_nodes$, Integer $T$, float $t$}
\State $transaction\_indexes \gets \{\}$
\ForEach{$pattern \in TPatterns$}
	\State $indexes \gets [\:]$
	\State $k \gets T - (|pattern| - min\_pattern\_nodes) \times (T \times (1 - t)) / (max\_pattern\_nodes - min\_pattern\_nodes)$
	\For{$i \gets 0 \text{ to } k - 1$}
	\State $indexes[i] \gets RandomInt(0, T - 1)$
	\EndFor
...
\end{algorithm}
\\
Finally, the actual transactions are generated and this sequence of steps is repeated for each transaction: Firstly, a pattern list is created and all the patterns which contain the transaction index in their index list generated with Algorithm~\ref{generate_indexes} are appended to it. This list will be named $chosen\_patterns$. Secondly, some $TransactionTree$ nodes are generated each with $n$ - 3 randomly generated attributes plus a unique \texttt{rid} field. The number of generated nodes depends on the size of $chosen_pattern$ list as these nodes serve the only purpose of adding random nodes to the transactions.
Thirdly, the randomly generated nodes and the $chosen\_patterns$ elements are merged together into one list which will be called $nodes\_to\_append$ and a root for the transaction is chosen randomly from this list. Furthermore, another list called $current\_tree$ is created and its purpose is to store the nodes which are already part of the transaction; the root node, together with all the pattern nodes if a pattern was chosen as root, is appended to this list.
