...
\end{algorithm}
\\
Finally, the actual transactions are generated and this sequence of steps is repeated for each transaction: Firstly, a pattern list is created and all the patterns which contain the transaction index in their index list generated with Algorithm~\ref{generate_indexes} are appended to it. This list will be named $chosen\_patterns$.
Secondly, some $TransactionTree$ nodes are generated each with $n$ - 3 randomly generated attributes, plus a unique \texttt{rid} field. The number of generated nodes depends on the size of $chosen\_pattern$ list, as these nodes serve the only purpose of adding random nodes to the transactions.
\begin{algorithm}
\caption{Generate the transactions}
\label{generate_indexes}
\begin{algorithmic}[1]
\Function{GenerateTransactions}{TransactionTree[] $TPatterns$, Dict(TransactionTree, Integer) $transaction\_indexes$, Integer $T$}
\For{$t\_index \gets 0 \text{ to } T - 1$}
    \State $chosen\_patterns \gets []$
    \State $chosen\_pattern\_index \gets 0$
    \ForEach{$pattern \in TPatterns$}
        \For{$i \gets 0 \text{ to } |transaction\_indexes[pattern]| - 1$}
            \If{$transaction\_indexes[pattern][i] == index$}
                \State $chosen\_patterns[chosen\_pattern\_index] \gets pattern$
                \State $chosen\_pattern\_index++$
            \EndIf
        \EndFor
    \EndFor
    \ForEach{$pattern \in chosen\_patterns$}
