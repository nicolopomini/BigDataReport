...
\item a threshold $t$ that indicates at least how many times a pattern must appear to be considered a frequent pattern, this is also an integer.
\end{itemize} 
...
Next, a dictionary is created to determine in which transactions each pattern will appear. The dictionary uses the generated patterns as keys, and integer lists as values. The integer lists contain the indexes of the trees in which the pattern (its key) needs to be present. For example, if the algorithm is generating transaction number $k$ and the integer lists associated with patterns $x$ and $y$ contain $k$, then both $x$ and $y$ will be appended into transaction number $k$. The number of transactions in which each pattern will appear is determined by the pattern size, meaning that shorter patterns will appear in more often compared to bigger patterns. Furthermore, by definition, the patterns should be frequent, thus a minimum number of transactions in which each pattern will appear is guaranteed. Once the number of times each pattern should appear over all transactions is determined, the actual transactions in which each of them will appear are chosen randomly, meaning that a pattern may appear more than one time in a given transaction.
\begin{algorithm}
\caption{Choose transactions for patterns.}
\label{generate_node}
\begin{algorithmic}[1]
\Function{GenerateIndexesDictionary}{TransactionTree[] $generated\_patterns$, Integer $max\_pattern\_nodes$, Integer $min\_pattern\_nodes$, Integer $total\_transactions$}
\State $pattern\_transaction\_indexes \gets \{\}$
\ForEach{$pattern \in generated\_patterns$}
	\State $tree\_indexes \gets [\:]$
	\State $k \gets total\_transactions - (|pattern| - min\_pattern\_nodes)$
	\For{$i \gets 1 \text{ to } k$}
	\EndFor
\EndFor
\EndFunction
...
\end{algorithm}
\\
Finally, the actual transactions are generated. For each transaction to be created a random number of $TransactionTree$ nodes are generated based on how many patterns each transaction will contain. These nodes are not part of any pattern and serve the only purpose to add random data to the transactions. Next, a root for each transaction is chosen
