...
\begin{algorithmic}[1]
\Function{translatePatterns}{PatternTree $t$, PatternTree $p$, Integer $n$, String[] $\boldsymbol{a}$}
\State $rid \gets \text{ \texttt{random\_string() }}$
\State $fields \gets t_{fields}$
\For{$i \gets 1 \text{ to } n$}
	\If{$\boldsymbol{a}_i \notin fields$}
		\State $fields[\boldsymbol{a}_i] \gets \text{\texttt{random\_string()}}$
	\EndIf
\EndFor
\State $children \gets \{\}$
...
\begin{algorithmic}[1]
\Function{GenerateRandomNodes}{TransactionTree[] $chosen\_patterns$, float $l$, int $n$, String[] $\boldsymbol{a}$}
\State $random\_nodes \gets []$
\For{$i \gets 0 \text{ to } 1 + l \times (1 + |chose\_patterns|)$}
    \State $rid \gets \text{ \texttt{random\_string() }}$
